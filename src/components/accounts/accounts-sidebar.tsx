...
function Donut({ slices }: { slices: AllocationSlice[] }) {
  ...
  if (total <= 0) {
    return (
      <div className="mx-auto flex size-[140px] items-center justify-center rounded-full bg-[var(--ds-gray-100)]">
  ...
  return (
    <div
      className="relative mx-auto size-[140px] rounded-full"
...
export function AccountsSidebar({
  ...
  return (
    <aside className="space-y-4 xl:sticky xl:top-14">
      <section className="rounded-[16px] bg-[var(--ds-background-elevated)] p-4 ds-border sm:p-5">
        <div className="mb-3 flex items-center justify-between gap-2">
          <div className="flex items-center gap-1.5">
            <Sparkles size={14} className="text-[var(--ds-focus-color)]" />
          ...
          <Link
            href="/ai"
            className="text-xs font-medium text-[var(--ds-focus-color)] hover:underline"
          >
            View all
          </Link>
        </div>
        <div className="space-y-2">
          {insights.map((insight) => {
            const body = (
              <div className="flex gap-2.5 rounded-[10px] bg-[var(--ds-background-100)] px-3 py-2.5 transition-colors hover:bg-[var(--ds-gray-100)]">
                <StatusDot tone={insight.tone} className="mt-1" />
                <div className="min-w-0">
                  <p className="text-xs font-medium text-[var(--ds-gray-1000)]">
                    {insight.title}
                  </p>
                  <p className="mt-0.5 text-[11px] leading-4 text-[var(--ds-gray-700)]">
                    {insight.detail}
                  </p>
                </div>
              </div>
      ...
      <section className="rounded-[16px] bg-[var(--ds-background-elevated)] p-4 ds-border sm:p-5">
        <h2 className="mb-4 text-sm font-semibold text-[var(--ds-gray-1000)]">
          Asset Allocation
        </h2>
          ...
          <ul className="space-y-3">
            {activity.map((item) => (
              <li key={item.id} className="flex items-start gap-2.5">
                <span
                  className="mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-full text-[10px] font-semibold"
                  style={{
                ...
                <div className="shrink-0 text-right">
                  ...
                  <p className="text-[10px] text-[var(--ds-gray-700)]">
